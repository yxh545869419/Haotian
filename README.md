...
在深度分析阶段，程序会先把目标仓库克隆到 `TMP_REPO_DIR` 下的临时目录，在本地做有边界的 probe，再把分析快照写入运行产物和报告数据。分析结束后会尝试删除临时 clone，通常会成功；如果清理失败，临时工作区可能会保留，并会通过 cleanup warnings / cleanup_completed state 暴露。如果仓库预算用尽、克隆失败或探测失败，流程会自动降级为 fallback analysis。
...
- `MAX_DEEP_ANALYSIS_REPOS`：每轮允许进入深度分析的仓库数量
...
如果报告里出现 fallback analysis，说明至少部分贡献证据来自 fallback analysis。优先查看 `analysis_depth`、`matched_files`、`evidence_snippets`，再结合 `probe_summary` 判断这次保留了多少证据、是否需要重新分析。

最终生成的 Markdown / JSON 报告会把这些快照渲染成 evidence-backed sections，方便直接追溯每个分类结论依赖了哪些仓库证据。
